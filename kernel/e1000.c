...
e1000_recv(void)
{
  ...
  // This is where the next packet would go.
  acquire(&e1000_lock);
  ...
  while (rx_ring[i].status & E1000_RXD_STAT_DD)
  {
    ...
    // allocate a new buffer to receive the next packet.
    rx_bufs[i] = kalloc();
    if (!rx_bufs[i])
    ...
    // tell the e1000 that this buffer is available
    regs[E1000_RDT] = i;

    // pass the packet up the stack for processing.
    net_rx(buf, len);
