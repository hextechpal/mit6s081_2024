...
pagetable_t proc_pagetable(struct proc *p) {
    ...
    // Initialize usyscall struct
    struct usyscall *us = (struct usyscall *)usyscall_page;
    us->pid = p->pid;

    // Map page read-only
    if (mappages(pagetable, USYSCALL, PGSIZE, (uint64)usyscall_page, PTE_R | PTE_U) < 0) {
        kfree(usyscall_page);
        uvmfree(pagetable, 0);
        return 0;
    }
