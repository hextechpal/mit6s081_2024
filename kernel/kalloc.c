...
void kinit() {
    ...
    freerange(end, (void *)SUPBASE);
    freesuperrange((void *)SUPBASE, (void *)PHYSTOP);
}
...
void kfree(void *pa) {
    ...
    if (((uint64)pa % PGSIZE) != 0 || (char *)pa < end || (uint64)pa >= SUPBASE)
        panic("kfree");
...
void superfree(void *pa) {
    ...
    if (((uint64)pa % SUPERPGSIZE) != 0 || (uint64)pa < SUPBASE || (uint64)pa >= PHYSTOP)
        panic("superfree");
